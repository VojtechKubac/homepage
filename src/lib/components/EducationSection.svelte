...
<section id="education" class="py-20 px-4 sm:px-6 lg:px-8">
      ...
      {#each education as edu, i (i)}
        <div class="animate-fade-in-up" style="animation-delay: {i * 100}ms">
          <div class="border-l-4 border-purple-600 pl-6 py-2">
            ...
            <p class="text-lg text-purple-600 dark:text-purple-400 font-semibold">{edu.school}</p>
            <p class="text-sm text-slate-500 dark:text-slate-400 mb-3">{edu.year}</p>
            <p class="text-slate-700 dark:text-slate-300">{edu.description}</p>
          </div>
        </div>
      {/each}
    </div>
